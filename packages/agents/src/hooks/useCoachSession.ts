...
export interface UseCoachSessionConfig {
  apiBaseUrl?: string;
}
...
export function useCoachSession(config?: UseCoachSessionConfig) {
  ...
  const startCoach = useCallback(
    async ({ context, audioElement }: CoachSessionOptions) => {
      ...
      const res = await fetch(`${base}/api/session/coach`, { method: "POST" });
      if (!res.ok) return;
