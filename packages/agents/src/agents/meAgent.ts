...
import { createTools, type ToolsConfig } from "./tools";
...
export function createMeAgent(ctx: MeAgentContext, config?: ToolsConfig) {
  const sharedTools = createTools(config);
  const allTools = [suggest_response, flag_warning, ...sharedTools] as ToolDefinition[];
